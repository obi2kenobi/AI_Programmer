# Endpoint: `PS_PowerBI_T5107_SHA`

- URL: `https://api.businesscentral.dynamics.com/v2.0/4d4b10ed-d04a-455b-80e1-aaf7ba4ee0d8/Production/ODataV4/Company('GRUPPO%20CAMARLINGHI%20S.P.A')/PS_PowerBI_T5107_SHA`
- Righe campione lette: 3
- Campi trovati: 185

> Stato: **mappato** (da verificare con riscontro: interfaccia BC / gestionale / totali noti).

| Campo | Tipo | Esempio | Significato | Verificato |
|---|---|---|---|---|
| `@odata.etag` | string | W/"JzIwOzE4MjkyNzk4NjczMTc5OTAyODU3MT... |  | ☐ |
| `Document_Type` | string | Quote |  | ☐ |
| `No` | string | 24OFFV-000001 |  | ☐ |
| `Doc_No_Occurrence` | int | 1 |  | ☐ |
| `Version_No` | int | 1 |  | ☐ |
| `_x0033_rd_Party_Loader_No` | string |  |  | ☐ |
| `_x0033_rd_Party_Loader_Type` | string |   |  | ☐ |
| `Activity_Code` | string |  |  | ☐ |
| `Additional_Information` | string |  |  | ☐ |
| `Additional_Instructions` | string |  |  | ☐ |
| `Additional_Notes` | string |  |  | ☐ |
| `Allow_Line_Disc` | bool | True |  | ☐ |
| `Amount` | float | 654.92 |  | ☐ |
| `Amount_Including_VAT` | int | 799 |  | ☐ |
| `Applies_to_Doc_No` | string |  |  | ☐ |
| `Applies_to_Doc_Type` | string |   |  | ☐ |
| `Applies_to_ID` | string |  |  | ☐ |
| `Applies_to_Occurrence_No` | int | 0 |  | ☐ |
| `Archived_By` | string | LUCA |  | ☐ |
| `Area` | string |  |  | ☐ |
| `Assigned_User_ID` | string |  |  | ☐ |
| `Bal_Account_No` | string |  |  | ☐ |
| `Bal_Account_Type` | string | G/L Account |  | ☐ |
| `Bank_Account` | string |  |  | ☐ |
| `Bill_to_Address` | string | Koper, Pristaniska ulica 3, |  | ☐ |
| `Bill_to_Address_2` | string |  |  | ☐ |
| `Bill_to_City` | string | Capodistria |  | ☐ |
| `Bill_to_Contact` | string |  |  | ☐ |
| `Bill_to_Contact_No` | string |  |  | ☐ |
| `Bill_to_Country_Region_Code` | string | SI |  | ☐ |
| `Bill_to_County` | string |  |  | ☐ |
| `Bill_to_Customer_No` | string | 500207 |  | ☐ |
| `Bill_to_IC_Partner_Code` | string |  |  | ☐ |
| `Bill_to_Name` | string | Antonio Lipari |  | ☐ |
| `Bill_to_Name_2` | string |  |  | ☐ |
| `Bill_to_Post_Code` | string | 6000 KOPER |  | ☐ |
| `Campaign_No` | string |  |  | ☐ |
| `Combine_Shipments` | bool | False |  | ☐ |
| `Comment` | bool | False |  | ☐ |
| `Completely_Shipped` | bool | False |  | ☐ |
| `Compress_Prepayment` | bool | True |  | ☐ |
| `Correction` | bool | False |  | ☐ |
| `Cumulative_Bank_Receipts` | bool | False |  | ☐ |
| `Currency_Code` | string |  |  | ☐ |
| `Currency_Factor` | int | 0 |  | ☐ |
| `Cust_Item_Disc_Gr` | string |  |  | ☐ |
| `Customer_Posting_Group` | string | PRIV |  | ☐ |
| `Date_Archived` | string | 2024-06-18 |  | ☐ |
| `Date_of_Birth` | string | 0001-01-01 |  | ☐ |
| `Dimension_Set_ID` | int | 1 |  | ☐ |
| `Document_Date` | string | 2024-03-18 |  | ☐ |
| `Due_Date` | string | 0001-01-01 |  | ☐ |
| `EU_3_Party_Trade` | bool | False |  | ☐ |
| `Exit_Point` | string | EXW |  | ☐ |
| `External_Document_No` | string |  |  | ☐ |
| `First_Name` | string |  |  | ☐ |
| `Fiscal_Code` | string |  |  | ☐ |
| `Gen_Bus_Posting_Group` | string | CLI_NAZ |  | ☐ |
| `Get_Shipment_Used` | bool | False |  | ☐ |
| `IC_Direction` | string | Outgoing |  | ☐ |
| `IC_Status` | string | New |  | ☐ |
| `Individual_Person` | bool | False |  | ☐ |
| `Interaction_Exist` | bool | False |  | ☐ |
| `Invoice` | bool | False |  | ☐ |
| `Invoice_Disc_Code` | string | 500207 |  | ☐ |
| `Invoice_Discount_Calculation` | string | None |  | ☐ |
| `Invoice_Discount_Value` | int | 0 |  | ☐ |
| `Language_Code` | string | ITA |  | ☐ |
| `Last_Name` | string |  |  | ☐ |
| `Last_Posting_No` | string |  |  | ☐ |
| `Last_Prepayment_No` | string |  |  | ☐ |
| `Last_Prepmt_Cr_Memo_No` | string |  |  | ☐ |
| `Last_Return_Receipt_No` | string |  |  | ☐ |
| `Last_Shipping_No` | string |  |  | ☐ |
| `Late_Order_Shipping` | bool | True |  | ☐ |
| `Location_Code` | string | PRINCIPALE |  | ☐ |
| `No_Printed` | int | 5 |  | ☐ |
| `No_Series` | string | V-OFFER |  | ☐ |
| `No_of_Archived_Versions` | int | 2 |  | ☐ |
| `On_Hold` | string |  |  | ☐ |
| `Operation_Occurred_Date` | string | 2024-03-18 |  | ☐ |
| `Operation_Type` | string | FT-VEN-IT+ |  | ☐ |
| `Opportunity_No` | string |  |  | ☐ |
| `Order_Class` | string |  |  | ☐ |
| `Order_Date` | string | 2024-03-18 |  | ☐ |
| `Outbound_Whse_Handling_Time` | string |  |  | ☐ |
| `Package_Tracking_No` | string |  |  | ☐ |
| `Payment_Percent` | int | 0 |  | ☐ |
| `Payment_Discount_Percent` | int | 0 |  | ☐ |
| `Payment_Method_Code` | string | BNKDOMCONV |  | ☐ |
| `Payment_Terms_Code` | string | ORDINE |  | ☐ |
| `Place_of_Birth` | string |  |  | ☐ |
| `Pmt_Discount_Date` | string | 0001-01-01 |  | ☐ |
| `Posting_Date` | string | 0001-01-01 |  | ☐ |
| `Posting_Description` | string | Offerta 24OFFV-000001 |  | ☐ |
| `Posting_No` | string |  |  | ☐ |
| `Posting_No_Series` | string | FT-VEN-IT+ |  | ☐ |
| `Posting_from_Whse_Ref` | int | 0 |  | ☐ |
| `Prepayment_Percent` | int | 100 |  | ☐ |
| `Prepayment_Due_Date` | string | 2024-03-31 |  | ☐ |
| `Prepayment_No` | string |  |  | ☐ |
| `Prepayment_No_Series` | string | FT-VEN-IT+ |  | ☐ |
| `Prepmt_CM_Refers_to_Period` | string |   |  | ☐ |
| `Prepmt_Cr_Memo_No` | string |  |  | ☐ |
| `Prepmt_Cr_Memo_No_Series` | string | FT-VEN-IT+ |  | ☐ |
| `Prepmt_Payment_Discount_Percent` | int | 0 |  | ☐ |
| `Prepmt_Payment_Terms_Code` | string | ORDINE |  | ☐ |
| `Prepmt_Pmt_Discount_Date` | string | 2024-03-18 |  | ☐ |
| `Prepmt_Posting_Description` | string |  |  | ☐ |
| `Price_Calculation_Method` | string | Lowest Price |  | ☐ |
| `Price_Group_Code` | string | LCDL |  | ☐ |
| `Prices_Including_VAT` | bool | False |  | ☐ |
| `Promised_Delivery_Date` | string | 0001-01-01 |  | ☐ |
| `Quote_Accepted` | bool | False |  | ☐ |
| `Quote_Accepted_Date` | string | 0001-01-01 |  | ☐ |
| `Quote_Sent_to_Customer` | string | 0001-01-01T00:00:00Z |  | ☐ |
| `Quote_Valid_Until_Date` | string | 0001-01-01 |  | ☐ |
| `Reason_Code` | string |  |  | ☐ |
| `Receive` | bool | False |  | ☐ |
| `Refers_to_Period` | string |   |  | ☐ |
| `Requested_Delivery_Date` | string | 0001-01-01 |  | ☐ |
| `Reserve` | string | Optional |  | ☐ |
| `Resident` | string | Resident |  | ☐ |
| `Responsibility_Center` | string |  |  | ☐ |
| `Return_Receipt_No` | string |  |  | ☐ |
| `Return_Receipt_No_Series` | string |  |  | ☐ |
| `Sales_Quote_No` | string |  |  | ☐ |
| `Salesperson_Code` | string |  |  | ☐ |
| `Sell_to_Address` | string | Koper, Pristaniska ulica 3, |  | ☐ |
| `Sell_to_Address_2` | string |  |  | ☐ |
| `Sell_to_City` | string | Capodistria |  | ☐ |
| `Sell_to_Contact` | string |  |  | ☐ |
| `Sell_to_Contact_No` | string |  |  | ☐ |
| `Sell_to_Country_Region_Code` | string | SI |  | ☐ |
| `Sell_to_County` | string |  |  | ☐ |
| `Sell_to_Customer_Name` | string | Antonio Lipari |  | ☐ |
| `Sell_to_Customer_Name_2` | string |  |  | ☐ |
| `Sell_to_Customer_No` | string | 500207 |  | ☐ |
| `Sell_to_Customer_Template_Code` | string |  |  | ☐ |
| `Sell_to_E_Mail` | string |  |  | ☐ |
| `Sell_to_IC_Partner_Code` | string |  |  | ☐ |
| `Sell_to_Phone_No` | string |  |  | ☐ |
| `Sell_to_Post_Code` | string | 6000 KOPER |  | ☐ |
| `Send_IC_Document` | bool | False |  | ☐ |
| `Service_Tariff_No` | string |  |  | ☐ |
| `Ship` | bool | False |  | ☐ |
| `Ship_to_Address` | string | Koper, Pristaniska ulica 3, |  | ☐ |
| `Ship_to_Address_2` | string |  |  | ☐ |
| `Ship_to_City` | string | Capodistria |  | ☐ |
| `Ship_to_Code` | string |  |  | ☐ |
| `Ship_to_Contact` | string |  |  | ☐ |
| `Ship_to_Country_Region_Code` | string | SI |  | ☐ |
| `Ship_to_County` | string |  |  | ☐ |
| `Ship_to_Name` | string | Antonio Lipari |  | ☐ |
| `Ship_to_Name_2` | string |  |  | ☐ |
| `Ship_to_Post_Code` | string | 6000 KOPER |  | ☐ |
| `Shipment_Date` | string | 2024-03-18 |  | ☐ |
| `Shipment_Method_Code` | string | EXW |  | ☐ |
| `Shipping_Advice` | string | Partial |  | ☐ |
| `Shipping_Agent_Code` | string |  |  | ☐ |
| `Shipping_Agent_Service_Code` | string |  |  | ☐ |
| `Shipping_No` | string |  |  | ☐ |
| `Shipping_No_Series` | string | V-SPED |  | ☐ |
| `Shipping_Time` | string |  |  | ☐ |
| `Shortcut_Dimension_1_Code` | string | ARRG |  | ☐ |
| `Shortcut_Dimension_2_Code` | string |  |  | ☐ |
| `Status` | string | Open |  | ☐ |
| `TDD_Prepared_By` | string |  |  | ☐ |
| `Tax_Area_Code` | string |  |  | ☐ |
| `Tax_Liable` | bool | False |  | ☐ |
| `Tax_Representative_No` | string |  |  | ☐ |
| `Tax_Representative_Type` | string |   |  | ☐ |
| `Time_Archived` | string | 13:07:45.207 |  | ☐ |
| `Transaction_Specification` | string |  |  | ☐ |
| `Transaction_Type` | string |  |  | ☐ |
| `Transport_Method` | string |  |  | ☐ |
| `VAT_Base_Discount_Percent` | int | 0 |  | ☐ |
| `VAT_Bus_Posting_Group` | string | CLI_NAZ |  | ☐ |
| `VAT_Country_Region_Code` | string | SI |  | ☐ |
| `VAT_Registration_No` | string | SI46883304 |  | ☐ |
| `Your_Reference` | string |  |  | ☐ |
| `Date_Filter` | string |  |  | ☐ |
| `Location_Filter` | string |  |  | ☐ |
| `Work_Description@odata.mediaEditLink` | string | https://api.businesscentral.dynamics.... |  | ☐ |
| `Work_Description@odata.mediaReadLink` | string | https://api.businesscentral.dynamics.... |  | ☐ |
