...
appsscript.json:9
dependency-risk
La libreria esterna DATO-RIMOSSOScript e' collegata con developmentMode: true in
...
forzaInvioDATO-RIMOSSODaEditor() è una funzione a zero parametri che invia email REALI
a un fornitore esterno (DATO-RIMOSSO), senza conferma, dry-run o controllo TEST_MODE,
ed è elencata nel menu Esegui dell'editor accanto a funzioni diagnostiche sicure (es.
verificaLibreriaDATO-RIMOSSO()).
Scenario di fallimento ▸
MEDIUM
NON VERIFICATO
BCSetup.gs:569
...
(salvataggio CSV su Drive righe 930-944, email Fornitore-N IO righe 1021-1024, email
...
DATO-RIMOSSO) non viene mai loggato in ORDER_ERROR_LOG e il fallimento è
...
test-false-safety
testFase4_Fornitore-NIoEmail() (Test 3), nel file che si dichiara 'SAFE', invia una email VERA
alla casella di produzione info@esempio.example quando
...
correctness
migrazioneEmailFornitore-NIo() è una funzione di scrittura reale su 'main' in un file che si
...
dashboard.html:504
config-duplication
L'indirizzo email di destinazione per l'invio forzato a DATO-RIMOSSO è scritto come testo
