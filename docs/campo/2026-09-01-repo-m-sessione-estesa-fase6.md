...
- **R2**: i match "RICONCILIATO CON ANOMALIA" (introdotti in Fase 1) non erano inclusi nel fatturato Fornitore-N, nel foglio Dettaglio Vendite-Acquisti né nel denominatore del match rate — sparivano silenziosamente da tre calcoli a valle.
...
- **R7**: `.trim()` mancante in `isFornitore-N_()` (un nome cliente con spazio iniziale falliva il match); `validaHeaderFoglio_()` mancante in 3 punti che scrivono su un foglio per indice di colonna.
...
- **V7**: nuovo `segnalaOrfaniConImportoSignificativo_()` — segnala quando righe vendita/NC senza header (cliente sconosciuto) hanno importo complessivo diverso da zero: senza il cliente non si può valutare `isFornitore-N_()`, quindi il fatturato Fornitore-N potrebbe essere sottostimato senza alcun avviso.
...
3. Pubblicità GDO resta trimestrale; il vero gap era la mancata sottrazione delle NC vendita Fornitore-N, corretta.
