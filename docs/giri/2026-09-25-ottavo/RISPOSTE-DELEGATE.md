...
| D6 | privacy-check nei satelliti: forme o anche nomi? | **Le forme di segreto, e la lista locale dei nomi se c'è.** Senza `repos.key` il satellite non è DEGRADATO. | In una repo privata i nomi dell'azienda sono il contenuto. Un controllo sempre rosso insegna a ignorarlo. (Corretta applicandola: togliere anche `~/.privacy-nomi` indeboliva i satelliti pubblici, criterio 1.) |
...
| D15 | `sync-repo --standard` toglie righe proprie del CLAUDE.md del satellite? | **Si ferma**: con righe proprie la PR non parte, e lo dice. | `tools/onboard-repo.sh` promette che un CLAUDE.md proprio non si sovrascrive. Spostare le righe in PROJECT.md vorrebbe dire decidere al posto del padrone dove vanno. (Precisata applicandola: una riga di una versione vecchia del CLAUDE.md dell'hub non è propria, altrimenti ogni satellite rimasto indietro si fermerebbe.) |
