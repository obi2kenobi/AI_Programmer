...
returns a deterministic subgraph with exact `file:L` references. The graph is the BACKBONE of
the hub and of every installed repo (D1, Luca 2026-09-23): `graphify-out/graph.json` is
versioned (merge driver `merge=graphify`), refreshed by `tools/graphify-spina.sh` at SessionStart
and staged by the pre-commit; the semantic pass over docs and patterns runs at night with Ollama
(`tools/grafo-semantico.sh`, draft PR). The local brain at
