...
[ "$RC" -eq 0 ] && ! grep -cw 'night/grafo-2026-09-28' <<<"$RAMI" >/dev/null && [ "${NCOMMIT:-0}" -eq 2 ] && ! grep -c '^pr create' "$T/gh.log" >/dev/null \
  && ok "D35: con una PR del grafo aperta il grafo nuovo va sul suo ramo, niente PR nuova" || ko "D35: rc=$RC, rami «${RAMI}», commit sul ramo aperto ${NCOMMIT:-?}, $(grep -c '^pr create' "$T/gh.log") PR create"
: > "$T/gh.log"
