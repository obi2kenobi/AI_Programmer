...
| 2026-09-23 | Censimento E-002 dei BANCHI: `echo … \| grep -q` sotto pipefail in 250 siti di 68 file di `tests/` (misura: `grep -lE pipefail tests/*.sh \| xargs grep -nE '(echo\|printf)[^\|]*\| *grep -q'`). Ognuno puo' dare un rosso a caso sotto carico — la suite notturna gira proprio sotto carico. Curati i 2 che hanno morso (E-042) | la cura e' meccanica (`grep -q … <<<"$X"`) ma tocca 66 file: e' il lavoro della caccia notturna, un banco per finestra, con il cricchetto che lo tiene fermo | un banco curato entra nella lista `CURATI` di `tests/test-e002-banchi-curati.sh`; si salda quando la lista copre tutti ⏳ IN ATTESA: la caccia notturna, un banco per finestra |
