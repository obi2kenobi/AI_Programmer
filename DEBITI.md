...
| 2026-08-21 | Mascherare segreti negli output del gate (pattern: segreto-come-impronta) | miglioramento suggerito dal raccolto AI_Develop, non urgente (output locali) | al prossimo giro su morning-gate |
...
| Data | Scorciatoia | Perché rimandata | Quando si salda |
|---|---|---|---|
| 2026-08-21 | `night-shift/gate-esito.sh` registra due volte lo stesso esito su righe diverse quando esistono PIÙ righe pendenti (senza esito) per lo stesso repo+PR — riprodotto dal vivo su una copia del vero `metrics/gate.csv` (non l'originale, mai toccato): `gate-esito.sh obi2kenobi/AI_Develop 369 merge` due volte di seguito scrive "merge" su due righe diverse (righe 7 e 9 della copia), invece di respingere la seconda come "già registrato". Causa: lo script cerca "l'ultima riga ancora pendente" ad ogni chiamata, non "esiste già un esito registrato per questo repo+PR". Scenario realistico, non costruito ad arte: più notti sullo stesso PR aperto producono più righe nuove-formato ancora vuote prima che un umano registri l'esito finale | il fix ovvio (respingere se un esito esiste già per repo+PR) romperebbe il caso legittimo opposto — un PR con esito `commessa` che riceve poi un esito `merge` su una riga successiva dopo la correzione. Distinguere "doppia registrazione per errore" da "secondo esito legittimo dopo un ciclo correttivo" è una decisione sul significato di "esito", non un fix meccanico | quando si decide la semantica esatta (es. un flag per riga che marca "superseded", o un controllo su `gh pr view --json state` invece di dedurre dal solo CSV) — richiede il sì di Luca prima di toccare uno script che scrive metriche già in uso |
