...
| 2026-09-23 | I banchi di propagazione `tests/test-bootstrap-*-propagation.sh` e `tests/test-onboard-*-propagation.sh`, e `tests/test-night-shift-design-gate.sh` / `tests/test-morning-gate-issue-num.sh`, RIFANNO a mano la logica che dicono di provare: con la copia di `tools/bootstrap-app.sh` commentata, o con il gate del Design spento in `night-shift/night-shift.sh`, restano verdi (provato dalla lente dei test finti). Per l'onboarding il presidio vero ora c'e' (`tests/test-onboard-repo.sh`, end-to-end, skill personalizzata che deve sopravvivere — sabotaggio → rosso); per bootstrap e per il gate del Design no | serve un banco end-to-end di `bootstrap-app.sh` con gh/git finti, e l'estrazione della funzione del gate dal turno: lavoro di un giro dedicato, non di una revisione | al prossimo giro su bootstrap-app o sul gate del Design: banco che esegue il codice vero, poi i banchi-copia si cancellano |
...
| 2026-09-23 | Due banchi portano ancora dipendenze d'ambiente: `tests/test-backup-config.sh` forza «gh assente» con PATH=/usr/bin:/bin (dove su Linux gh puo' esserci) e conta i salti come ok; `tests/test-ask-wrappers.sh` scrive log fissi in /tmp (due esecuzioni parallele si pestano) | minori, nessun verde falso misurato oggi | al prossimo giro su quei test |
