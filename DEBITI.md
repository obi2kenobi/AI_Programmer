...
| 2026-09-23 (curati nella notte i log fissi in /tmp di `tests/test-ask-wrappers.sh` e `tests/test-stdin-timeout.sh`, Q23) | Resta: `tests/test-backup-config.sh` forza «gh assente» con PATH=/usr/bin:/bin (dove su Linux gh puo' esserci) e conta i salti come ok | minore, nessun verde falso misurato oggi | al prossimo giro su quel test |
